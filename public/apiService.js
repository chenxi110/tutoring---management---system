class ApiService {
    ...
    async uploadCourseFile(file, classId) {
        ...
        var url = this.baseURL + '/course-files/upload';
    ...
    async submitCourseFile(file, classId, studentId) {
        ...
        var url = this.baseURL + '/course-files/submit';
        var res = await fetch(url, {
            method: 'POST',
            headers: this.token ? { 'Authorization': 'Bearer ' + this.token } : {},
            body: formData
    ...
    async getCourseFiles(classId) {
        return this.request('GET', '/course-files/list?classId=' + classId);
    }
    getCourseFileDownloadUrl(fileId) {
        return this.baseURL + '/course-files/download/' + fileId + '?token=' + encodeURIComponent(this.token);
    }
