...
/**
 * 家长端 AI 学习助手接口
 * 全部接口仅家长角色可访问，且校验孩子归属（只能看自己绑定的孩子）
 */
@RestController
...
public class ParentAiController {
    ...
    public Map<String, Object> children(HttpServletRequest req) {
        if (!RoleAccess.isParent(req)) return forbid();
        Long parentId = RoleAccess.getUserId(req);
        Map<String, Object> r = new HashMap<>();
        r.put("code", 200);
        r.put("data", parentAiService.getChildren(parentId));
        return r;
    }
    ...
    public Map<String, Object> overview(@RequestParam Long studentId, HttpServletRequest req) {
        if (!RoleAccess.isParent(req)) return forbid();
        return parentAiService.getOverview(RoleAccess.getUserId(req), studentId);
    }
    ...
    public Map<String, Object> wrongQuestions(@RequestParam Long studentId,
        ...
        if (!RoleAccess.isParent(req)) return forbid();
        return parentAiService.getWrongQuestions(RoleAccess.getUserId(req), studentId, subject);
    }
    ...
    public Map<String, Object> submissions(@RequestParam Long studentId, HttpServletRequest req) {
        if (!RoleAccess.isParent(req)) return forbid();
        return parentAiService.getSubmissions(RoleAccess.getUserId(req), studentId);
    }
    ...
    public Map<String, Object> paper(@PathVariable Long submissionId, HttpServletRequest req) {
        if (!RoleAccess.isParent(req)) return forbid();
        return parentAiService.getPaper(RoleAccess.getUserId(req), submissionId);
    }
    ...
    public Map<String, Object> analysis(@RequestBody Map<String, Object> body, HttpServletRequest req) {
        ...
        return parentAiService.generateAnalysis(RoleAccess.getUserId(req), studentId, classId, reportType);
    }
    ...
    public Map<String, Object> reports(@RequestParam Long studentId, HttpServletRequest req) {
        if (!RoleAccess.isParent(req)) return forbid();
        return parentAiService.listReports(RoleAccess.getUserId(req), studentId);
    }
    ...
    public Map<String, Object> chat(@RequestBody Map<String, Object> body, HttpServletRequest req) {
        ...
        return parentAiService.chat(RoleAccess.getUserId(req), studentId, message);
    }
    ...
    public Map<String, Object> generateQuestions(@RequestBody Map<String, Object> body, HttpServletRequest req) {
        ...
        return parentAiService.generateQuestions(RoleAccess.getUserId(req), studentId, topic, count, types);
    }
    ...
    public Map<String, Object> saveLearningPlan(@RequestBody Map<String, Object> body, HttpServletRequest req) {
        ...
        return parentAiService.saveLearningPlan(RoleAccess.getUserId(req), studentId, focus);
    }
    ...
    public Map<String, Object> learningPlans(@RequestParam Long studentId, HttpServletRequest req) {
        if (!RoleAccess.isParent(req)) return forbid();
        return parentAiService.listLearningPlans(RoleAccess.getUserId(req), studentId);
    }
    ...
    public Map<String, Object> updateProgress(@PathVariable Long planId,
        ...
        return parentAiService.updatePlanProgress(RoleAccess.getUserId(req), planId, progress, status);
    }
}
